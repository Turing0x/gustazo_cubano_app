...
class GeneratePdfOrder {
  static Future<Map<String, dynamic>> generate(OrderInvoice invoice) async {
    try {
      ...
      final fileName =
          'ORDEN-${invoice.orderNumber.replaceAll(' ', '')}-${invoice.orderDate.split(' - ')[0].replaceAll('/', '-')}';
      ...
      build: (context) => [
        pw.Container(
            ...
            child: pw.Column(children: [
              topRow(invoice.title, invoice.address, image, '${invoice.pendingNumber} - ${invoice.orderNumber}',
              ...
              pw.Row(
                  ...
                  children: [signatures('Facturado'), signatures('Despachado'), signatures('Recibido')]),
            ]))
      ],
    );
  }

  static Row topRow(String title, String address, MemoryImage image, String number, String orderDate, String method) {
